...
export async function POST(req: NextRequest) {
  try {
    ...
    if (apiKey) {
      ...
      const systemPrompt = `
        You are an expert venture capitalist, CTO, and pitch designer.
              ...
              "chart_data": [
                {
                  ...
                  "color": "Optional color string (e.g. '#00f2fe')"
                }
              ]
            }
          ]
      ...
      console.log("GEMINI_API_KEY is missing. Running investor demo simulator.");
      
      const techListStr = techList.length > 0 ? techList.join(", ") : "cloud software stacks";
      
      await new Promise((resolve) => setTimeout(resolve, 1200));

      return NextResponse.json({
        executive_summary: `Managed venture proposal for '${title}'. By packaging local sandboxes, async syncing architectures, and specialized ${techListStr} integrations, we establish an unfair speed moat in SaaS infrastructure. We resolve key query delays, offering sub-15ms local latency profiles while keeping data structures fully compliant.`,
        investment_highlights: [
          `Highly scaleable B2B SaaS infrastructure targeting a $45B market opportunity`,
          `Proprietary local WASM worker compilation layer offering 10x performance gains`,
          `85% Gross Margin target via decentralized off-chain ledger sync models`
        ],
        pitch_deck: [
          {
            slide_id: 1,
            ...
            chart_type: "bar",
            chart_data: [
              { label: "Standard SaaS", value: 150.0, color: "#f15bb5" },
              { label: `${title} App`, value: 15.0, color: "#00f2fe" }
            ]
          },
          {
            slide_id: 2,
            ...
            subtitle: `A localized WASM runtime layer auto-reconciling node files.`,
            bullet_points: [
          ...
          },
          {
            ...
            chart_type: "pie",
            chart_data: [
              { name: "TAM ($B)", value: 45.0, color: "#00f2fe" },
              { name: "SAM ($B)", value: 8.5, color: "#9b5de5" },
              { name: "SOM ($B)", value: 1.2, color: "#00f5d4" }
            ]
          },
          {
            slide_id: 4,
            ...
            subtitle: `Leveraging advanced client compiling and async syncing stacks.`,
            bullet_points: [
              "WASM compiler sandboxing executing safe client-side code execution",
              "FastAPI async endpoint routers managing distributed node state configurations",
              `Seamless optimization layers targeting specialized ${techListStr} integrations`
            ],
            chart_type: "list",
            chart_data: []
          },
          ...
          },
          {
            ...
            bullet_points: [
              "Resilient local synchronization compared to legacy server query lag-times",
            ...
            chart_type: "radar",
            chart_data: [
              { label: "Sync Speed", value: 95.0, color: "#00f2fe" },
              { label: "Cost Moat", value: 90.0, color: "#9b5de5" },
              { label: "Offline Safety", value: 98.0, color: "#00f5d4" }
            ]
          },
          {
            slide_id: 7,
          ...
          },
          {
            ...
            chart_type: "bar",
            chart_data: [
              { label: "Year 1 ($M)", value: 1.2, color: "#00f5d4" },
              { label: "Year 3 ($M)", value: 8.5, color: "#9b5de5" },
              { label: "Year 5 ($M)", value: 24.0, color: "#00f2fe" }
            ]
          }
        ]
      });
    ...
    console.error("Error in investor-demo route:", error);
